# -*- coding: utf-8 -*-
"""
正式なウォークフォワード検証（tasks/handoff_next_session.md「OOS逆転の警戒」の原則に従う）。

これまでの検証（backtest_range_position_grouping.py等）は全期間プールでの集計であり、
backtest_benchmark_and_timing_check.pyの「時期集中除外」は簡易確認にとどまっていた。
本スクリプトは、過去に週足RSIフィルター等で有効だった手法（複数の非重複ウィンドウに
分割し、各ウィンドウ内で「閾値30%以下」グループと「同ウィンドウの全シグナル日」を
比較する、同一母集団内比較）を適用する。

前提: 他のbacktest_*.pyと同じデータキャッシュ・bot/pbr_signal.pyのロジックを使う。

使い方:
    core16_dividend_botディレクトリで `python backtest_walkforward_threshold.py` を実行する。
"""
...
OUTPUT_PATH = DATA_CACHE / "walkforward_threshold_result.json"

HORIZONS_YEARS = [1, 3, 5]
FORWARD_DATE_TOLERANCE_DAYS = 10
THRESHOLD = 30  # 検証方法4の推奨閾値

# 非重複の暦年ウィンドウ。5年後リターンが計算できるのはエントリー日から5年経過後のみのため、
# 直近数年（2022年以降）は5年後カラムがNaNだらけになる（フォワードデータ不足）。
# これは正直な制約として結果にそのまま表示する（無理に埋めない）。
